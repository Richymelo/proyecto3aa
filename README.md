...
```bash
gcc -Wall -Wextra -std=c11 $(pkg-config --cflags gtk+-3.0) -o proyecto-3aa proyecto-3aa.c $(pkg-config --libs gtk+-3.0)
```
...
- `proyecto-3aa.c` - Código fuente principal
- `proyecto-3aa.glade` - Archivo de interfaz gráfica (Glade)
...
- El programa requiere que el archivo `proyecto-3aa.glade` esté en el mismo directorio
